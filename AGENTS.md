...
## Current Status
SPEC-001 (Phase 1) is approved. Currently on branch `feat/spec-001-m1-extraction`, starting Milestone M1 (structured game extraction — `x4cat` extraction tooling, SQLite schema/tables, data ingest with domain invariants, and golden fixtures). The retrieval-grounding evaluation harness (part of M6) does not exist yet -- nothing should be treated as fully gated on grounding until it does.
