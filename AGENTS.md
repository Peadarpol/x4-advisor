...
## Current Status
SPEC-001 (Phase 1) is approved. Milestone M1 (structured game extraction) is DELIVERED (PR #4). Milestone M2 (structured query engine for 4 query templates) is DELIVERED (PR #5). Milestone M3 (unstructured ingestion pipeline) is DELIVERED (PR #6). Currently on branch `feat/spec-001-m4-vector-retrieval`, starting Milestone M4 (unstructured vector retrieval engine). The retrieval-grounding evaluation harness (part of M6) does not exist yet -- nothing should be treated as fully gated on grounding until it does.
